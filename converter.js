...
const fastRef = {
    fc(s) {
        return [["color", getTailwindColor(s)]]
    },
    bg(s) {
        return [["background", getTailwindColor(s)]]
    },
    cm(s) {
        return handlers.colorMatch(s)
    },
}
const callableRef = {
    pill(color) {
...
const fastRE = reWrap(fastRef, /^($1)(.+)/)

function runCallable(ref, { name, args, kwargs }) {
    const base = ref[name]
    const fn = base
    return fn(...args)
}
...
function fast(s) {
    ...
    const mapper = (item) => {
        ...
        if (m = match(item, fastRE)) {
            return fparse(fastRef, ...m)
        }
        if (m = parseCallable(item)) {
            return fparse(callableRef, m.name, ...m.args)
        }
        ...
        throw new Error(`"${item}" is invalid key @ csx.fast`)
    }

    const base = items.map(mapper).flat()
    const payload = fix(base)
    return dict(payload)
}

console.log(fast("cmb3 py5 pill(blue, asdf  :   1) mx15 my50 m30"))
