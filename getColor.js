...
function getTailwindColor(key, index) {
    ...
    if (index == null) {
        ;[key, index] = parseTailwindColor(key)
    }
    return assets.tailwindColors[key][getColorIndex(index)]
}

function getMaterialColor(colorKey, colorIndex, comment = false) {
    const value = assets.indexedMaterialColors[colorKey][colorIndex]
    const suffix = ` /* ${colorKey}-${colorIndex || 0} */`
    return comment ? value + suffix : value
}

function parseMaterialColor(s) {
    const [a, b] = match(s, /^([a-zA-Z]+)(\d*)/)
    const colorKey = assets.indexedMaterialColorsAliases[a] || a
    const colorIndex = b === "" ? 5 : Number(b) - 1
    return [colorKey, colorIndex]
}

function parseTailwindColor(s) {
    const [a, b] = match(s, /^([a-zA-Z]+)(\d*)/)
    const colorKey = a.length == 1
        ? assets.roygbiv.find((color) => color.startsWith(a))
        : s
    return [colorKey, getColorIndex(b)]
}
...
const getColor = getTailwindColor
